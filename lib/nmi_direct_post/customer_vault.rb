...
module NmiDirectPost
  class CustomerVaultNotFoundError < StandardError;
  end

  class CustomerVaultInvalidPostActionError < StandardError;
  end
  ...
  class CustomerVault < Base
    ...
    def find!
      begin
        ...
        customer = self.class.get(self.class.all_params(safe_params, self))['customer_vault']
        if !customer.nil? && customer.length == 1
          @customer_vault_id = customer[0]['customer_vault_id'] # This assumes there is only 1 result.
        else
    ...
    def find_or_create
      begin
        ...
        vault = self.class.get(self.class.all_params(safe_params, self))['customer_vault']
        if !vault
          self.save!
        elsif vault.is_a?(Array) && vault.length == 1
          @customer_vault_id = self.class.get(self.class.all_params(safe_params, self))['customer_vault'][0]['customer_vault_id'] # This assumes there is only 1 result.
        elsif vault.is_a?(Array) && vault.length > 1
          raise "Multiple Email, First, Last Name matches. Clean Data!"
        end
        # TODO: When there are multiple results, we don't know which one you want.  Maybe raise an error in that case?
        reload
      ensure
